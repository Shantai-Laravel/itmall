...
$(document).on('click', '.cartHere .plus', function(event){
...
$(document).on('click', '.cartHere .minus', function(event){
  let product_id = $(this).data('product_id');
  let subproduct_id = $(this).data('subproduct_id');
  let order_id = $(this).data('order_id');
  let user_id = $('input[name="user_id"]').val();
...
$(document).on('click', '.modalToCart', function(event){
  ...
  $.ajax({
    ...
    data: { product_id: product_id, order_id: order_id, user_id: user_id},
    success: function (data) {
      let res = JSON.parse(data);
      $('.cartHere').html(res.block);
    },
...
$(document).on('click', '.searchProductByCode', function(event){
    ...
    $.ajax({
        ...
        success: function(data) {
          ...
          let a = $('<a>').attr('class', 'modalToCart').attr('data-id', data.product.id).attr('data-order_id', order_id);
          $("body").append($(a));
          $(a).trigger("click");
        },
        error: function (data) {
...
$(document).ready(function () {
  ...
  $(document).on("change", 'select[name="orderProducts_return"]', function(e){
      ...
      if(confirm('Вы хотите добавить этот товар в возврат?')) {
          $.ajax({
            ...
            data: {orderProduct_id: orderProduct_id, product_id: product_id, subproduct_id: subproduct_id, path: path, return_id: return_id},
            success: function(data) {
              let res = JSON.parse(data);
              $('.cartReturn').html(res.return);
              $('.orders').html(res.orders);
  ...
  $(document).on('click', '.cartReturn .plus', function(event){
    let product_id = $(this).data('product_id');
    let subproduct_id = $(this).data('subproduct_id');
  ...
  $(document).on('click', '.cartReturn .minus', function(event){
    let product_id = $(this).data('product_id');
    let subproduct_id = $(this).data('subproduct_id');
    let orderProduct_id = $(this).data('order_product_id');
    let return_id = $(this).data('return_id');
