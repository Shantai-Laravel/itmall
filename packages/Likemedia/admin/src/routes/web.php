...
Route::group(['middleware' => ['web']], function ()
{
    ...
    Route::group(['namespace' => $namespace, 'prefix' => 'back', 'middleware' => 'auth'], function ()
    {
        ...
        Route::get('/test', function(){
            // $propertiesMultidatas = App\Models\PropertyMultiData::get();
            //
            // if (count($propertiesMultidatas) > 0) {
            //     foreach ($propertiesMultidatas as $key => $propertiesMultidata) {
            //         App\Models\PropertyMultiDataTranslation::create([
            //             'property_multidata_id' => $propertiesMultidata->id,
            //             'lang_id' => 6,
            //             'name' => $propertiesMultidata->translationByLanguage(1)->first()->name,
            //             'value' => $propertiesMultidata->translationByLanguage(1)->first()->value,
            //         ]);
            //     }
            // }

            // $propertiesValues = App\Models\PropertyValue::get();
            //
            // if (count($propertiesValues) > 0) {
            //     foreach ($propertiesValues as $key => $propertiesValue) {
            //         App\Models\PropertyValueTranslation::create([
            //             'property_values_id' => $propertiesValue->id,
            //             'lang_id' => 6,
            //             'value' => $propertiesValue->translationByLanguage(1)->first()->value,
            //         ]);
            //     }
            // }
        });
        ...
        Route::post('/promotions/changePosition', 'PromotionsController@changePosition');
